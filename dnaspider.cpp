...
wstring editor1 = L"Notepad", editor = L"Visual Studio Code", db = L"db.txt - ", se = L"se.txt - ", editorSe = L"\u25cf " + se + editor, editorDb = L"\u25cf " + db + editor;
...
void showOutsMsg(Multi multi, wstring s, wstring w, wstring s1 = L"", bool b = 0) {
	...
	wcout << s;
	SetConsoleOutputCP(CP_UTF8); wcout.imbue(locale(wcout.getloc(), new codecvt_utf8_utf16<wchar_t>)); for (; x < w.length(); ++x) {
		if (b && w[x] == '\\') {
			t = 0;
			switch (w[x + 1]) {
			case'd': //out <+>
