...
struct Multi {
	...
	int cx, cy, speed_;
	bool esc, br;//break
	Multi() :qp{}, r{}, g{}, b{}, a{}, x{}, m{}, l{}, t{ tail }, q{ qq }, s{ strand }, i_{}, icp{}, cx{}, cy{}, speed_{}, esc{}, br{} {}
...
void scanDb() {
	...
	wstring cell; relink = 0; while (getline(f, cell, multiLineDelim[0])) { //cout << cell << endl;
		...
		if (auto a = cell.substr(0, sv.size() + !close_ctrl_mode), b = sv.substr(0, sv.size() - close_ctrl_mode);
			...
			tail = isVar(tail); //<r:>
			if (multiStrand) { multi.t = tail; mainn.getStrand(cell); tail = multi.t; }
			...
			for (i = 0; i < tail.length(); ++i) {
				if (multiStrand) { multi.t = tail; multi.i_ = i; if (strand + L">" == sv || strand == sv && !noClearStrand) strand = L""; }//multiStrand variant clear
				if (speed > 0) { if (sleep) { if (multiStrand) { re = multi.t = tail; multi.i_ = i; multi.q = qq; multi.setSpeed(speed); } Sleep(speed); if (multiStrand) { tail = multi.t; i = multi.i_; qq = multi.q; } } sleep = 1; }
				if (!NoEscapeOrPause) {
...
#pragma endregion
	for (;; Sleep(frequency)) {
		...
		if (RSHIFTLSHIFT_Only && !strand[0] && Kb_QQ_k > 0 && GetAsyncKeyState(Kb_QQ_k)) {
			...
			if (Kb_QQ_i > 1) { Kb_QQ_i = 0; kb(VK_BACK); kb(VK_BACK); GetAsyncKeyState(VK_BACK); strand = qScanOnly ? L"<" : L""; ++rri; prints(); clearAllKeys(); continue; }
			Sleep(frequency / 3);
			continue;
		}
