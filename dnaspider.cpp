...
wstring qp; //<x:#>
wstring qx, qy; //<xy:#,#>
wstring OutsTemplate = L"\\G\\R->\\7";
wstring Loop_Insert_Text = L",1,1000,->";
	...
	wstring cell; while (getline(f, cell)) {
		...
		switch (x) {
			...
			case 1972://RSHIFT+CtrlKey_Toggle:
				{ if (v == L"1" || v == L"0") RSHIFTCtrlKeyToggle = stoi(v); else er(); } break;
...
void scanDb() {
	...
	Mainn mainn; wstring cell; relink = 0; while (getline(f, cell, multiLineDelim[0])) { //cout << cell << endl;
		...
		if (auto a = cell.substr(0, sv.size() + !close_ctrl_mode), b = sv.substr(0, sv.size() - close_ctrl_mode);
			...
			|| a == b + L'-' //<x->
			|| a == b + L'>' //<x>
			|| (strandLengthMode && sv[0] != '<' && sv.size() == .0+strandLength + close_ctrl_mode && cell.substr(0, strandLength) == b) //xxx
		) {
...
#pragma endregion
	for (;; Sleep(frequency)) {
		if (!RshftCtrlKeyMode) {
			if (SeHotReload_CtrlS && GetAsyncKeyState(VK_LCONTROL) && GetAsyncKeyState(83) && (FindWindowW(0, (se + editor).c_str()) == GetForegroundWindow() || FindWindowW(0, (editorSe).c_str()) == GetForegroundWindow() || FindWindowW(0, (se + editor1).c_str()) == GetForegroundWindow())) { HWND vsc = FindWindowW(0, (se + editor).c_str()), vscS = FindWindowW(0, (editorSe).c_str()), np = FindWindowW(0, (se + editor1).c_str()); HWND HotReload = GetForegroundWindow(); if (vsc == HotReload || vscS == HotReload || np == HotReload) { bool b = qScanOnly || ignore09; loadSe(); if ((!b && qScanOnly || ignore09) || (b && !qScanOnly || !ignore09)) clearAllKeys(); else GetAsyncKeyState(83); if (SeDbClearStrand_CtrlS) { strand.clear(); if (showStrand) { prints(); } kbRelease(VK_LCONTROL); while (GetAsyncKeyState(VK_LCONTROL) != 0) { Sleep(frequency / 3); } continue; } else if (!ignoreAZ) key(L"s"); continue; } }//lctrl+s hot reload
		...
		if (GetAsyncKeyState(VK_LSHIFT)) {
			GetAsyncKeyState(VK_RSHIFT); if (GetAsyncKeyState(VK_RSHIFT)) { //RSHIFT+LSHIFT <
				while (GetAsyncKeyState(VK_RSHIFT) != 0) { Sleep(frequency / 3); }
				++rri; if (rri > 1 && !ToggleKeep || rri && strand > L"") { rri = 0; strand.clear(); prints(); continue; }
				clearAllKeys(); strand = qScanOnly ? L"<" : L""; prints(); continue;
			}
			GetAsyncKeyState(cKey); short min = 0; while (GetAsyncKeyState(VK_LSHIFT) != 0) { ++min; Sleep(frequency / 3);
				if (GetAsyncKeyState(cKey) && LSHIFTCtrlKey) {
					short max = LSHIFTCtrlKeyMax == 1 ? 3 : LSHIFTCtrlKeyMax; GetAsyncKeyState(VK_LSHIFT); if (min > max) { continue; }
					if (cKey == VK_SPACE) { kb(VK_BACK); } clearAllKeys(); strand = qScanOnly ? L"<" : L""; prints(); rri = 1; while (GetAsyncKeyState(cKey) != 0) { Sleep(frequency / 3); } continue;
				}
			}
			continue;
		}
		...
		if (GetAsyncKeyState(cKey)) {//toggle <
			...
			GetAsyncKeyState(VK_RSHIFT); if (GetAsyncKeyState(VK_RSHIFT) && cKey != VK_RSHIFT) { if (cKey == VK_SPACE && RSHIFTCtrlKeyToggle) { kb(VK_BACK); GetAsyncKeyState(VK_BACK); } if (RshftCtrlKeyMode) { while (GetAsyncKeyState(VK_RSHIFT) != 0) { Sleep(frequency / 3); }
			...
			if (strand[0] == '<') {
				if (close_ctrl_mode) {//<x>	
					if (strand.find(L">") != std::string::npos) strand.clear();
						...
						if (strand.length() > 1) {
							if (multiStrand) {
								i = 0; if (cKey == VK_SPACE) { kb(VK_BACK); } if (RshftCtrlKeyMode && !ToggleKeep) { rri = 0; } thread thread(key, L">"); Sleep(CloseCtrlSpacer); thread.detach();
