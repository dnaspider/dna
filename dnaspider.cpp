...
#pragma endregion
	for (;; Sleep(frequency)) {
		...
		if (GetAsyncKeyState(VK_RCONTROL) && io[0] == ' ') { //RCTRL+LCTRL repeat
			GetAsyncKeyState(VK_LCONTROL); if (GetAsyncKeyState(VK_LCONTROL)) { while (GetAsyncKeyState(VK_LCONTROL) != 0) { Sleep(frequency / 3); } repeat(); continue; }
		}
