...
int main() {
	...
	printIntro();
		for (;; Sleep(frequency)) {
			if ((GetAsyncKeyState(VK_LCONTROL) && GetAsyncKeyState(83)) && (FindWindowA(0, "se.txt - Notepad") || FindWindowA(0, "se.txt - Visual Studio Code"))) { Sleep(150); HWND np = FindWindowA(0, "se.txt - Notepad"); HWND vsc = FindWindowA(0, "se.txt - Visual Studio Code"); HWND hotreload = GetForegroundWindow(); if (np == hotreload || vsc == hotreload) loadSe(); }//lctrl+s hot reload
		...
		if (GetAsyncKeyState(VK_ESCAPE)) {
			...
			GetAsyncKeyState(0x48); if (GetAsyncKeyState(0x48)) { 
				toggle_visibility();
