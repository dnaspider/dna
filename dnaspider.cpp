...
void scanDb() {
	...
	if (reTail.substr(0,8) != L"<repeat>") pre = reTail;
}
...
#pragma endregion
	for (;; Sleep(frequency)) {
		...
		if (GetAsyncKeyState(cKey)) {//toggle <
			GetAsyncKeyState(VK_LCONTROL); if (GetAsyncKeyState(VK_LCONTROL) && cKey == 163) {
			...
			while (GetAsyncKeyState(cKey) != 0) Sleep(frequency / 3);
			if (strand[0] == '<') {
