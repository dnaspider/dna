...
wstring editor = L"Notepad", editor1 = L"Visual Studio Code", editorSe = L"se - Notepad", editorDb = L"db - Notepad", db = L"db.txt - ", se = L"se.txt - ";
...
void loadSe() {
	...
	wstring cell; while (getline(f, cell)) {
		...
		switch (x) {
			case 1449://MaxLengthClear:
				{ if (check_if_num(v) != L"") MaxLengthClear = stoi(v); else er(); } break;
...
void printSe() {
	...
	if (showSettings) {
		...
		cout << "MaxLengthClear: " << MaxLengthClear << '\n';
	...
	if (hDC != NULL) {
		...
		if (color != CLR_INVALID) {
			...
			if (GetAsyncKeyState(VK_LSHIFT))
				...
				tail = L"<shift>,<shift->rgb:" + c + (Loop_Insert ? L",>" : L">");
			re = tail;
		}
	}
	return L"";
...
void scanDb() {
	if (strand.length() >= (MaxLengthClear + 1 + close_ctrl_mode + close_ctrl_mode)) { strand.clear(); return; }
	if (close_ctrl_mode) {
		if (strand.length() == 0) {
			if (re == L"")
				return;
		...
		if (re > L"" && strand == L"" || strand > L"" && cell > L"" && (close_ctrl_mode && cell.substr(0, sv.size()) == strand || (!strandLengthMode || sv[0] == '<') && cell.substr(0, sv.size()) == sv.substr(0, sv.size() - 1) + L":" || cell.substr(0, sv.size()) == sv.substr(0, sv.size() - 1) + L"-" || cell.substr(0, sv.size() + 1) == sv.substr(0, sv.size() - 1) + L":>" || cell.substr(0, sv.size() + 1) == sv.substr(0, sv.size() - 1) + L"->") || cell.substr(0, sv.size() + 1) == strand + L">" || cell.substr(0, sv.size() + 1) == strand + L":" || cell.substr(0, sv.size() + 1) == strand + L"-" || (strandLengthMode && cell.substr(0, strandLength) == strand && cell[0] != '<') || close_ctrl_mode && strandLengthMode && strand[0] != '<' && cell.substr(0, sv.size() - 1) == sv.substr(0, sv.size() - 1)) { //found i>o, i:o, i-o, i:>o, i->o || i>o, i:o, i-o || io || io
			...
			if (re > L"" && strand == L"") {
				...
				if (re[0] == '>') {
					...
					else if (re.substr(0, 21) == L"><shift>,<shift->app:") { wstring x = L"><shift>,<shift->app:"; out(L"<alt><esc><alt-><,1>"); x += getAppT(); out(L"<shift><alt><esc><alt-><shift->"); re = x + (Loop_Insert ? L",>" : L">"); }
				}
			}
				...
				case'<':
					...
					case '~': 
						if (qqb(L"<~>")) {//manual set xy
							...
							NoEscapeOrPause = 0; rei();
						}
						else if (qqb(L"<~~esc>")) {
							NoEscapeOrPause = 1; rei();
						}
						else conn();
						break;
					case'+': //calc
					...
					case'D':
						...
						case 'B':
						case 'b':
							if (testqqb(L"<DB:") || testqqb(L"<db:") || testqqb(L"<db_:")) {//.h Database:
					...
					case'r':
						...
						case 'g':
							if (testqqb(L"<RGB~:") || testqqb(L"<RGB:") || testqqb(L"<rgb:")) { //<rgb:r g b,x,ms,link>
								...
								if (testqqb(L"<rgbxy>")) { getRGB(1, 1); rei(); break; }//rshift + esc + r
					...
					case'S':
						...
						case 'E':
						case 'e':
							if (testqqb(L"<SE:") || testqqb(L"<se:") || testqqb(L"<se_:")) {//.h Switch Settings: file
		...
		if (GetAsyncKeyState(VK_ESCAPE)) {
			...
			GetAsyncKeyState(82); if (GetAsyncKeyState(82)) {//esc + r: <rgb:>
				...
				if (GetAsyncKeyState(VK_LSHIFT)) {//lshift + esc + r
					out(L"<shift>,<shift->rgb: " + getRGB() + L" " + qx + L" " + qy + (Loop_Insert ? L",>" : L">")); continue;
				}
