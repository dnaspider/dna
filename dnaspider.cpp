...
#pragma endregion
	for (;; Sleep(frequency)) {
		if (SeHotReload_CtrlS && GetAsyncKeyState(VK_LCONTROL) && GetAsyncKeyState(83) && (FindWindowW(0, (se + editor).c_str()) == GetForegroundWindow() || FindWindowW(0, (se + editorSe).c_str()) == GetForegroundWindow() || FindWindowW(0, (editor1).c_str()) == GetForegroundWindow())) { HWND vsc = FindWindowW(0, (se + editor).c_str()), vscS = FindWindowW(0, (se + editorSe).c_str()), np = FindWindowW(0, (editor1).c_str()); HWND HotReload = GetForegroundWindow(); if (vsc == HotReload || vscS == HotReload || np == HotReload) { bool b = qScanOnly || ignore09; loadSe(); if ((!b && qScanOnly || ignore09) || (b && !qScanOnly || !ignore09)) clearAllKeys(); else GetAsyncKeyState(83); if (SeDbClearStrand_CtrlS) { strand.clear(); if (showStrand) { prints(); } continue; } else if (!ignoreAZ) key(L"s"); continue; } }//lctrl+s hot reload
		if (SeDbClearStrand_CtrlS && GetAsyncKeyState(VK_LCONTROL) && GetAsyncKeyState(83) && (FindWindowW(0, (db + editor).c_str()) == GetForegroundWindow() || FindWindowW(0, (db + editorDb).c_str()) == GetForegroundWindow() || FindWindowW(0, (editor1).c_str()) == GetForegroundWindow())) { GetAsyncKeyState(83); strand.clear(); if (showStrand) { prints(); } continue; }//clear
