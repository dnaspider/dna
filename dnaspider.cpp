...
void prints(bool clear = 0) { if (clear) strand.clear(); auto s = strand; s = s.replace(0, s.length(), L"\\G" + s + L"\\7");  if (showStrand) showOutsMsg(L"", OutsTemplate + s, L"\n", 1); }
...
void loadSe() {
	...
	wstring cell; while (getline(f, cell)) {
		...
		wstring v = (cell.substr(cell.find_first_of(L":\t ") + 1)); if (v.find_first_not_of(L"\t ") == string::npos) { if (se == L"io:") { io = L" "; } else if (se == L"OutsTemplate:") { OutsTemplate = L""; } else if (se == L"Kb_Key_Space:") { Kb_Key_Space = L" "; } else if (se == L"Loop_Insert_Text:") { Loop_Insert_Text = L""; } continue; } v = v.substr(v.find_first_not_of(L"\t "));
		...
		switch (x) {
			...
			case 1751://RSHIFT+CtrlKey_Mode:, RSHIFT+CtrlKey_Keep:
				{ if (se == L"RSHIFT+CtrlKey_Mode:") { if (v == L"1" || v == L"0") RshftCtrlKeyMode = stoi(v); else er(); }
				else if (se == L"RSHIFT+CtrlKey_Keep:") { if (v == L"1" || v == L"0") ToggleKeep = stoi(v); else er(); }
				} break;
			case 764://OutTabs:
...
wstring getRGB(bool b = 0, bool bg = 0) {
	...
	if (hDC != NULL) {
		...
		if (color != CLR_INVALID) {
			...
			if (b) {
				if (rs) {//rshift+r+esc
					auto r = L"<RGB~:" + c + L" " + to_wstring(x) + L" " + to_wstring(y) + (Loop_Insert_Text > L"" ? Loop_Insert_Text : L">");
					cbSet(r);
					return L"";
				}
...
void scanDb() {
	...
	Mainn mainn; wstring cell; relink = 0; while (getline(f, cell, multiLineDelim[0])) { //cout << cell << endl;
		...
		if (re > L"" && strand == L"" || strand > L"" && cell > L"" && (close_ctrl_mode && cell.substr(0, sv.size()) == strand || (!strandLengthMode || sv[0] == '<') && cell.substr(0, sv.size()) == sv.substr(0, sv.size() - 1) + io[0] || cell.substr(0, sv.size()) == sv.substr(0, sv.size() - 1) + L":" || cell.substr(0, sv.size()) == sv.substr(0, sv.size() - 1) + L"-" || cell.substr(0, sv.size() + 1) == sv.substr(0, sv.size() - 1) + L":>" || cell.substr(0, sv.size() + 1) == sv.substr(0, sv.size() - 1) + L"->") || cell.substr(0, sv.size() + 1) == strand + L">" || cell.substr(0, sv.size() + 1) == strand + L":" || cell.substr(0, sv.size() + 1) == strand + L"-" || cell.substr(0, sv.size() + 1) == strand + io[0] || (strandLengthMode && cell.substr(0, strandLength) == strand && cell[0] != '<') || close_ctrl_mode && strandLengthMode && strand[0] != '<' && cell.substr(0, sv.size() - 1) == sv.substr(0, sv.size() - 1)) { //found i>o, i:o, i-o, i:>o, i->o || i>o, i:o, i-o, i*o || io || io
...
#pragma endregion
	for (;; Sleep(frequency)) {
		if (!RshftCtrlKeyMode) {
			if (SeHotReload_CtrlS && GetAsyncKeyState(VK_LCONTROL) && GetAsyncKeyState(83) && (FindWindowW(0, (se + editor).c_str()) == GetForegroundWindow() || FindWindowW(0, (editorSe).c_str()) == GetForegroundWindow() || FindWindowW(0, (se + editor1).c_str()) == GetForegroundWindow())) { HWND vsc = FindWindowW(0, (se + editor).c_str()), vscS = FindWindowW(0, (se + editorSe).c_str()), np = FindWindowW(0, (editor1).c_str()); HWND HotReload = GetForegroundWindow(); if (vsc == HotReload || vscS == HotReload || np == HotReload) { bool b = qScanOnly || ignore09; loadSe(); if ((!b && qScanOnly || ignore09) || (b && !qScanOnly || !ignore09)) clearAllKeys(); else GetAsyncKeyState(83); if (SeDbClearStrand_CtrlS) { strand.clear(); if (showStrand) { prints(); } kbRelease(VK_LCONTROL); while (GetAsyncKeyState(VK_LCONTROL) != 0) { Sleep(frequency / 3); } continue; } else if (!ignoreAZ) key(L"s"); continue; } }//lctrl+s hot reload
		...
		if (GetAsyncKeyState(VK_RCONTROL) && io[0] == ' ') {
			GetAsyncKeyState(VK_LCONTROL); if (GetAsyncKeyState(VK_LCONTROL)) { while (GetAsyncKeyState(VK_LCONTROL) != 0) { Sleep(frequency / 3); } repeat(); continue; }//lctrl+rctrl repeat
			if (GetAsyncKeyState(83) && SeHotReload_CtrlS) {
				...
				{ loadSe(); clearAllKeys(); strand.clear(); prints(); kbRelease(VK_RCONTROL); while (GetAsyncKeyState(VK_RCONTROL) != 0) { Sleep(frequency / 3); } continue; }
				continue;
			}//rctrl+s
		}
		if (GetAsyncKeyState(cKey)) {//toggle <
			if (RshftCtrlKeyMode && rri && strand[0] == 0) continue;
