	...
	if (hDC != NULL) {
		...
		if (color != CLR_INVALID) {
			...
			if (GetAsyncKeyState(VK_LSHIFT))
				tail = L"<shift>,<shift->rgb:" + c + L" " + to_wstring(pt.x) + L" " + to_wstring(pt.y) + (Loop_Insert ? L",>" : L">");
			else
				...
				case'<':
					...
					case '^':
						if (qqb(L"<^>")) {//Toggle back after run
							...
							close_ctrl_mode = !close_ctrl_mode; qScanOnly = !qScanOnly; rei();
						}
						else conn();
						break;
					case '~': 
		...
		if (GetAsyncKeyState(VK_ESCAPE)) {
			...
			GetAsyncKeyState(82); if (GetAsyncKeyState(82)) {//esc + r: <rgb:>
				...
				if (GetAsyncKeyState(VK_LSHIFT)) {//lshift + esc + r
					out(L"<shift>,<shift->rgb: " + getRGB(1) + L" " + qx + L" " + qy + (Loop_Insert ? L",>" : L">")); continue;
				}
